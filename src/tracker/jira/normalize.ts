import type { Issue, BlockerRef } from "../issue.js";
import { issuePriority, normalizeLabels } from "../issue.js";

export const normalizeJiraIssue = (raw: Record<string, unknown>): Issue | null => {
  ...
  return {
    ...
    priority: issuePriority(
      (fields.priority as { id?: unknown } | undefined)?.id ?? (fields.priority as { name?: unknown } | undefined)?.name
    ),
    state: status,
    branch_name: null,
    url: null,
...
const blockers = (value: unknown): BlockerRef[] => {
    ...
    return [
      {
        ...
        identifier: typeof issue.key === "string" ? issue.key : null,
        state: readName(fields.status)
      }
    ];
  });
};
...
const readName = (value: unknown): string | null =>
  value && typeof value === "object" && typeof (value as { name?: unknown }).name === "string" ? (value as { name: string }).name : null;

const stringifyDescription = (value: unknown): string | null => {
