...
export const normalizeLinearIssue = (raw: Record<string, unknown>): Issue | null => {
  ...
  const labelNodes = readNodes(raw.labels)
    .map((node) => readName(node))
...
const readNodes = (value: unknown): Record<string, unknown>[] => {
