...
const commentsQuery = "query NorthstarLinearComments($issueId:String!,$first:Int!,$after:String){issue(id:$issueId){comments(first:$first,after:$after){nodes{id body createdAt user{name email displayName}} pageInfo{hasNextPage endCursor}}}}";
const createCommentMutation = "mutation NorthstarLinearCreateComment($issueId:String!,$body:String!){commentCreate(input:{issueId:$issueId,body:$body}){success comment{id body createdAt user{name email displayName}}}}";
...
export class LinearTracker implements Tracker {
  ...
  async createComment(issueId: string, body: string): Promise<TrackerComment | void> {
    ...
    return normalizeLinearComment((response as { data?: { commentCreate?: { comment?: unknown } } })?.data?.commentCreate?.comment) ?? undefined;
  }
  ...
  private async fetchCommentPages(issueId: string, variables: Record<string, unknown>, acc: TrackerComment[] = []): Promise<TrackerComment[]> {
    const response = await this.graphql(commentsQuery, variables);
    const page = readCommentPage(response);
    const comments = [...acc, ...readCommentNodes(response).map(normalizeLinearComment).filter((comment): comment is TrackerComment => comment !== null)];
    return page.hasNextPage && page.endCursor ? this.fetchCommentPages(issueId, { issueId, first: 50, after: page.endCursor }, comments) : comments;
  }
}
...
function readCommentNodes(response: unknown): unknown[] {
  const comments = response && typeof response === "object" ? (response as { data?: { issue?: { comments?: { nodes?: unknown[] } } } }).data?.issue?.comments : undefined;
  return Array.isArray(comments?.nodes) ? comments.nodes : [];
}

function readCommentPage(response: unknown): { hasNextPage: boolean; endCursor: string | null } {
  const page = response && typeof response === "object" ? (response as { data?: { issue?: { comments?: { pageInfo?: { hasNextPage?: unknown; endCursor?: unknown } } } } }).data?.issue?.comments?.pageInfo : undefined;
  return { hasNextPage: page?.hasNextPage === true, endCursor: typeof page?.endCursor === "string" ? page.endCursor : null };
}

function normalizeLinearComment(raw: unknown): TrackerComment | null {
