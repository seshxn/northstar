...
export function builtinTools(names: string[], workspacePath: string): Tool[] {
...
function readTool(root: string): Tool {
  return {
    name: "read_file",
...
function writeTool(root: string): Tool {
  return {
    name: "write_file",
...
function editTool(root: string): Tool {
  return {
    name: "edit_file",
...
function bashTool(root: string): Tool {
  return {
    name: "bash",
    ...
    execute: (args, ctx) => runBash(root, getString(args, "command"), Number((args as { timeout_ms?: unknown }).timeout_ms ?? 60_000), ctx.signal)
  };
}

function runBash(root: string, command: string, timeoutMs: number, signal: AbortSignal) {
  return new Promise<ReturnType<typeof jsonResult>>((resolvePromise, reject) => {
...
function getString(args: unknown, key: string): string {
  if (!args || typeof args !== "object" || typeof (args as Record<string, unknown>)[key] !== "string") throw new Error(`${key} must be a string`);
