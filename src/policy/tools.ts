...
export function filterToolsForIssue(tools: Tool[], policy: ToolPolicy, issue: Pick<Issue, "labels">): Tool[] {
...
function allowedSetForIssue(policy: ToolPolicy, issue: Pick<Issue, "labels">): Set<string> | null {
...
function disallowedSetForIssue(policy: ToolPolicy, issue: Pick<Issue, "labels">): Set<string> {
  return new Set([...policy.disallowed_tools, ...unionForLabels(policy.disallowed_tools_by_label, issue.labels)]);
}

function unionForLabels(rules: Record<string, string[]>, labels: string[]): Set<string> {
