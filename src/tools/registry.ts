...
export function buildTools(config: NorthstarConfig): Tool[] {
  const tools: Tool[] = [];
  ...
  if (config.integrations.jira_tools?.enabled) {
    tools.push(new JiraRestTool({ baseUrl: config.integrations.jira_tools.base_url ?? "", email: config.integrations.jira_tools.email, apiToken: config.integrations.jira_tools.api_token }));
  }
  ...
  if (config.integrations.confluence?.enabled) {
    tools.push(new ConfluencePageTool({ baseUrl: config.integrations.confluence.base_url ?? "", email: config.integrations.confluence.email, apiToken: config.integrations.confluence.api_token, defaultSpace: config.integrations.confluence.default_space }));
  }
  return tools;
}
